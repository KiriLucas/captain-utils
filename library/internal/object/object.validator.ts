...
export class ObjectUtilsValidator {
  ...
  static validateIsEmptyInput(input: unknown): void {
    if (
      TypeGuard.isBoolean(input) ||
      TypeGuard.isNumber(input) ||
      TypeGuard.isSymbol(input) ||
      TypeGuard.isBigInt(input) ||
      TypeGuard.isFunction(input)
    )
      throw new IsEmptyInvalidParametersException(input);
  }
}
